...
public sealed class AuthApiClient(HttpClient httpClient)
{
    ...
    public async Task<AuthResponse?> RegisterAsync(RegisterUserRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new InvalidOperationException(string.IsNullOrWhiteSpace(error) ? "Registration failed." : error.Trim());
        }
    ...
    public async Task<AuthResponse?> LoginAsync(LoginRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var error = await response.Content.ReadAsStringAsync(cancellationToken);
            throw new InvalidOperationException(string.IsNullOrWhiteSpace(error) ? "Invalid username or password." : error.Trim());
        }
